...
public class GuiNewSpaceRace extends GuiScreen implements ICheckBoxCallback, ITextBoxCallback {
    ...
    private boolean canPlayerEdit() {
        return this.mc.thePlayer.getGameProfile()
            .getName()
            .equals(
        ...
        if (this.initialized) {
            ...
            this.buttonList.add(
                new GuiElementGradientButton(
                    ...
                    50,
                    15,
                    this.currentState == EnumSpaceRaceGui.MAIN ? GCCoreUtil.translate("gui.spaceRace.create.close.name")
            ...
            switch (this.currentState) {
                ...
                case DESIGN_FLAG:
                    ...
                    this.flagDesignerMinX = this.width / 2 - this.spaceRaceData.getFlagData()
                        .getWidth() * (float) this.flagDesignerScale.x / 2;
                    this.flagDesignerMinY = this.height / 2 - this.spaceRaceData.getFlagData()
                        .getHeight() * (float) this.flagDesignerScale.y / 2;
                    this.flagDesignerWidth = this.spaceRaceData.getFlagData()
                        .getWidth() * (float) this.flagDesignerScale.x;
                    this.flagDesignerHeight = this.spaceRaceData.getFlagData()
                        .getHeight() * (float) this.flagDesignerScale.y;
                ...
                case CHANGE_TEAM_COLOR:
                    ...
                    this.sliderColorR.setSliderPos(
                        this.spaceRaceData.getTeamColor()
                            .floatX());
    ...
    public void updateScreen() {
        ...
        if (this.currentState == EnumSpaceRaceGui.ADD_PLAYER && this.gradientListAddPlayers != null
            ...
            for (final Object element : this.thePlayer.worldObj.playerEntities) {
                ...
                if (player.getDistanceSqToEntity(this.thePlayer) <= 25 * 25) {
                    ...
                    if (!this.spaceRaceData.getPlayerNames()
                        .contains(username)) {
                        playerNames.add(
                            new ListElement(
                                username,
                                this.recentlyInvited.containsKey(username) ? ColorUtil.to32BitColor(255, 250, 120, 0)
        ...
        if (this.currentState == EnumSpaceRaceGui.REMOVE_PLAYER && this.gradientListRemovePlayers != null
            && this.ticksPassed % 20 == 0) {
            final List<ListElement> playerNames = new ArrayList<>();
            for (int i = 1; i < this.spaceRaceData.getPlayerNames()
                .size(); i++) {
                final String playerName = this.spaceRaceData.getPlayerNames()
                    .get(i);
            ...
            if (this.canEdit && x >= this.flagDesignerMinX
                ...
                if (Mouse.isButtonDown(0)) {
                    if (this.checkboxEraser.isSelected != null && this.checkboxEraser.isSelected) {
                        ...
                        final Vector3 colorAt = this.spaceRaceData.getFlagData()
                            .getColorAt(unScaledX, unScaledY);
                    ...
                    && this.checkboxSelector.isSelected) {
                        if (this.selectionMinX != -1 && this.selectionMinY != -1
                            ...
                            this.selectionMaxX = Math.min(
                                unScaledX + 1,
                                this.spaceRaceData.getFlagData()
                                    .getWidth());
    ...
    private void setColor(int unScaledX, int unScaledY, Vector3 color) {
        if (this.selectionMaxX - this.selectionMinX > 0 && this.selectionMaxY - this.selectionMinY > 0) {
            if (unScaledX >= this.selectionMinX && unScaledX <= this.selectionMaxX - 1
                ...
                this.markDirty();
                this.spaceRaceData.getFlagData()
                    .setColorAt(unScaledX, unScaledY, color);
            }
        } else {
            this.markDirty();
            this.spaceRaceData.getFlagData()
    ...
    private void setColorWithBrushSize(int unScaledX, int unScaledY, Vector3 color, int brushSize) {
        for (int x = unScaledX - brushSize + 1; x < unScaledX + brushSize; x++) {
            for (int y = unScaledY - brushSize + 1; y < unScaledY + brushSize; y++) {
                if (x >= 0 && x < this.spaceRaceData.getFlagData()
                    .getWidth()
                    && y >= 0
                    && y < this.spaceRaceData.getFlagData()
                        .getHeight()) {
    ...
    public void sendSpaceRaceData() {
        if (this.canEdit) {
            ...
            objList.add(
                this.spaceRaceData.getPlayerNames()
                    .toArray(
                        new String[this.spaceRaceData.getPlayerNames()
                            .size()]));
        ...
        if (this.initialized) {
            ...
            switch (this.currentState) {
                ...
                case DESIGN_FLAG:
                    ...
                    for (int x = 0; x < this.spaceRaceData.getFlagData()
                        .getWidth(); x++) {
                        for (int y = 0; y < this.spaceRaceData.getFlagData()
                            .getHeight(); y++) {
                            final Vector3 color = this.spaceRaceData.getFlagData()
                                .getColorAt(x, y);
    ...
    private void drawColorButton() {
        ...
        if (this.canEdit) {
            this.fontRendererObj.drawString(
                ...
                this.buttonTeamColor_xPosition + this.buttonTeamColor_width / 2
                    - this.fontRendererObj
                        .getStringWidth(GCCoreUtil.translate("gui.spaceRace.create.changeColor.name.0")) / 2,
        ...
        this.fontRendererObj.drawString(
            ...
            this.buttonTeamColor_xPosition + this.buttonTeamColor_width / 2
                - this.fontRendererObj.getStringWidth(GCCoreUtil.translate("gui.spaceRace.create.changeColor.name.1"))
                    / 2,
        ...
        this.fontRendererObj.drawString(
            ...
            this.buttonTeamColor_xPosition + this.buttonTeamColor_width / 2
                - this.fontRendererObj.getStringWidth(GCCoreUtil.translate("gui.spaceRace.create.changeColor.name.2"))
                    / 2,
