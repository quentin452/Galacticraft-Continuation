...
public class GuiSlimelingFeed extends GuiScreen
{
    ...
    public GuiSlimelingFeed(final EntitySlimeling slimeling) {
    ...
    protected void actionPerformed(final GuiButton par1GuiButton) {
        if (par1GuiButton.enabled) {
            switch (par1GuiButton.id) {
                case 0: {
                    GalacticraftCore.packetPipeline.sendToServer((IPacket)new PacketSimpleMars(PacketSimpleMars.EnumSimplePacketMars.S_UPDATE_SLIMELING_DATA, new Object[] { this.slimeling.getEntityId(), 2, "" }));
                    break;
                }
                case 1: {
                    if (!this.slimeling.isInLove() && this.slimeling.isOwner((EntityLivingBase)this.mc.thePlayer) && this.slimeling.worldObj.isRemote) {
                        this.slimeling.func_146082_f((EntityPlayer)this.mc.thePlayer);
                    }
                    GalacticraftCore.packetPipeline.sendToServer((IPacket)new PacketSimpleMars(PacketSimpleMars.EnumSimplePacketMars.S_UPDATE_SLIMELING_DATA, new Object[] { this.slimeling.getEntityId(), 3, "" }));
                    break;
                }
                case 2: {
                    GalacticraftCore.packetPipeline.sendToServer((IPacket)new PacketSimpleMars(PacketSimpleMars.EnumSimplePacketMars.S_UPDATE_SLIMELING_DATA, new Object[] { this.slimeling.getEntityId(), 4, "" }));
                    break;
                }
                case 3: {
                    GalacticraftCore.packetPipeline.sendToServer((IPacket)new PacketSimpleMars(PacketSimpleMars.EnumSimplePacketMars.S_UPDATE_SLIMELING_DATA, new Object[] { this.slimeling.getEntityId(), 5, "" }));
                    break;
                }
            }
    ...
    static {
        slimelingPanelGui = new ResourceLocation("galacticraftmars", "textures/gui/slimelingPanel1.png");
        GuiSlimelingFeed.drawItems = new RenderItem();
    }
}
