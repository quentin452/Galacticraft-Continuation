...
public class GuiOxygenCollector extends GuiContainerGC {
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        final String status = GCCoreUtil.translate("gui.status.collecting.name") + ": "
            + (int) (0.5F
                + Math.min(this.collector.lastOxygenCollected * 20F, TileEntityOxygenCollector.OUTPUT_PER_TICK * 20F))
    ...
    private String getStatus() {
        ...
        if ((EnumColor.DARK_GREEN + GCCoreUtil.translate("gui.status.active.name")).equals(returnValue)
    ...
    protected void drawGuiContainerBackgroundLayer(float var1, int var2, int var3) {
        ...
        this.mc.getTextureManager()
            .bindTexture(GuiOxygenCollector.collectorTexture);
